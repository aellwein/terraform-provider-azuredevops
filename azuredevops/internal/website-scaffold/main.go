...
func main() {
	...
	resourceId := f.String("resource-id", "", "An Azure DevOps ID showing an example of how to Import this Resource")
	...
	if isResource && (resourceId == nil || *resourceId == "") {
		quitWithError("An example of an Azure DevOps ID must be specified via `-resource-id` when scaffolding for a Resource")
		return
	}
